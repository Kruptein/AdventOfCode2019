...
class RelativeIntcode(GeneratorIntCode):
    ...
    def get_mode(self, offset: int, modes: List[str], *, index_mode: bool = False) -> int:
        if len(modes) + 1 > offset and modes[offset - 1] == "2":
            if index_mode: return self.get(offset) + self.relative_base
...
if __name__ == "__main__":
    ...
    assert len(str(RelativeIntcode(split_row(int, "1102,34915192,34915192,7,4,7,99,0"), []).process()[0])) == 16
    assert RelativeIntcode(split_row(int, "104,1125899906842624,99"), []).process()[0] == 1125899906842624
